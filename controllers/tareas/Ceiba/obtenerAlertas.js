  ...
  try {
    ...
    const promesas = alarmasCeiba.map(async (alarma) => {
      ...
      const nuevaAlarma = await ADAM_alarmas_ceiba.create({
        ...
        serie: alarma.Device,
        inicio: moment(alarma.StartTime).subtract(3, 'hours').format("DD-MM-YYYY HH:mm:ss"), // Restar 4 horas y formatear la fecha
        fin: moment(alarma.EndTime).subtract(3, 'hours').format("DD-MM-YYYY HH:mm:ss"), // Restar 4 horas y formatear la fecha
        fecha_carga: moment(alarma.UploadTime).subtract(3, 'hours').format("DD-MM-YYYY HH:mm:ss"), // Restar 4 horas y formatear la fecha
    ...
    console.log("Todas las alarmas han sido procesadas y actualizadas");
  } catch (error) {
    console.error("Error procesando las alarmas", error);
  }
};
